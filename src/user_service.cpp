...
UserService::UserService(std::shared_ptr<MysqlConnection> sql_conn) : sql_conn_(std::move(sql_conn)) {
}
...
bool UserService::insert(std::string &mobile) {
    char sql_content[1024] = {0};
    sprintf(sql_content, "insert into userinfo(username, mobile) values(\"张三\", \"%s\")", mobile.c_str());
    SqlRecordSet record_set;
